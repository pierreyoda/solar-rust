    ...
    ($getter_name: ident, $enum_value: ident, $enum_type: ty) => (
        ...
        pub fn $getter_name<S: Into<String>>(&mut self, key: S) -> Option<&mut $enum_type> {
            ...
            if !self.properties.contains_key(&key_string) {
                let value: $enum_type = Default::default();
                self.properties.insert(key_string.clone(), ObjectProperty {
                    value: $enum_value(value),
                    description: "auto-generated on demand by ObjectRegister's mutable getter".into(),
                });
            }
...
mod tests {
    ...
    fn test_object_register() {
        ...
        register.get_int_mut("test_int").map(|mut i| *i += 30);
        assert_eq!(register.get_int("test_int"), Some(&42));

        assert_eq!(register.get_float("test_float"), None);
        register.get_float_mut("test_float").map(|mut f| *f = 3.14);
